...
| Placeholder | Resolved from `_refs/angular/core-version.md` field |
|---|---|
| `<CORE_UI_PACKAGE_NAME>` | `packageName` |
| `<CORE_VERSION>` | `currentVersion` |

> The actual values live ONLY in [`_refs/angular/core-version.md`](_refs/angular/core-version.md). Do not duplicate them here — the drift-check hook will block any literal version or any import statement that hardcodes the Core UI package name in this file.

**BEFORE generating any file**, the agent MUST:

1. Read [`_refs/angular/core-version.md`](_refs/angular/core-version.md) and extract `packageName` + `currentVersion`
...
Single-file bump: change `_refs/angular/core-version.md` and every future portal picks it up. Do NOT find-replace placeholders inside this reference file itself — they are intentional.

If `_refs/angular/core-version.md` is missing or malformed, STOP and ask the user — never guess the version or package name.
...
- Wire it in [`src/app/configurations/layout.configuration.ts`](_refs/angular/templates/init-portal-templates.md#layoutconfigurationts) as `sidebar.logoUrl: '/logo.png'` (template handles this).
...
- `_refs/angular/templates/init-portal-templates.md` is present (file-content templates)
- `_refs/angular/core-version.md` is present (packageName + currentVersion)
- `_refs/angular/assets/logo.png` is present (default brand logo)
...
All file-content templates referenced by the Generation Steps above live in [`_refs/angular/templates/init-portal-templates.md`](_refs/angular/templates/init-portal-templates.md). Read it when writing the corresponding files:

| File to generate | Section in templates ref |
|---|---|
| `package.json` | [`#packagejson`](_refs/angular/templates/init-portal-templates.md#packagejson) |
| `src/app/app.routes.ts` | [`#approutests`](_refs/angular/templates/init-portal-templates.md#approutests) |
| `src/main.ts` | [`#maints`](_refs/angular/templates/init-portal-templates.md#maints) — most rationale-heavy; preserve the inline `// Why ...` comments verbatim |
| `src/app/configurations/permission.configuration.ts` | [`#permissionconfigurationts-starter-default--disabled`](_refs/angular/templates/init-portal-templates.md#permissionconfigurationts-starter-default--disabled) |
| `src/app/configurations/layout.configuration.ts` | [`#layoutconfigurationts`](_refs/angular/templates/init-portal-templates.md#layoutconfigurationts) — wires `sidebar.logoUrl: '/logo.png'` |
