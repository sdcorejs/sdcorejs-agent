...
description: Use to build a REAL working contact form — client-side validation with zod + react-hook-form (or plain state), API route POST `/api/contact`, Resend (or SendGrid) email integration, in-memory rate limiting, bilingual error messages, accessible success/error UI. Replaces the fake `setTimeout` stub pattern of the `website_myhoa` baseline. Triggers - "contact form", "biểu mẫu liên hệ", "form không gửi email", "fix fake form", or automatic step of `07-write-code` for any site that includes a contact section. Bilingual (VI/EN).
...
## Purpose
A landing site without a working contact form is incomplete — leads vanish. The `website_myhoa` baseline ships a `setTimeout(1000)` stub that simulates submit. This skill ships a real form: zod validation, POST to API route, Resend email delivery, rate limit, bilingual UX, and accessibility.

## When invoked
- Automatic last step of "Full build" in `07-write-code`
...
Prerequisites:
- Contact email destination from `02-clarify-requirements` (`CONTACT_TO_EMAIL`)
- Resend account + API key (default email service) OR SendGrid alternative
- i18n messages exist (form labels, errors via `15-i18n`)
...
Add to `src/i18n/messages/vi.json`:
```json
{
  "contact": {
    "form": {
...
### MUST NOT
- Ship `setTimeout` fake submit (the website_myhoa bug we're fixing)
...
## Cross-references
- i18n messages: `15-i18n`
- Theme tokens (button color, success/danger): `11-theme`
- Section composition: `12-pages-and-blocks` (`<ContactForm />` rendered inside contact page or as section)
- Caching: API route is `force-dynamic` (not cached)
- Verification: `_shared/verify-before-done` includes "send-test-email succeeds end-to-end" as acceptance criterion
