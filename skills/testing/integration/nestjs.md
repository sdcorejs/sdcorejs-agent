...
description: Use to write integration tests for NestJS — `Test.createTestingModule` with real DI container, real service + repository, mocked HTTP for external services. Faster than e2e (no real HTTP server, in-memory DB) but exercises real NestJS wiring + real SQL via pg-mem. Inherits cross-track principles from `testing/philosophy.md`. Triggers - "integration test nestjs", "test service + repo", "test workflow logic with DB", or invocation after a service skill completes. Bilingual (VI/EN).
allowed-tools: Read, Write, Edit, Bash
---

# Testing — Integration (NestJS)
...
## When invoked
- After a service skill lands (e.g. workflow that orchestrates 3 repos)
- User says "test service + repo", "test transaction", "integration nestjs"
- Bug fix touching service / repo logic
...
- E2E (with HTTP): `testing/e2e/nestjs.md`
- Unit (mocked repos, no DB): `testing/unit/nestjs.md`
