...
- **Progress checklist (mandatory during write-code).** At the START of any code-writing run, create a `TodoWrite` checklist — one checkbox item per planned unit (file / screen / entity / module / page / pack step) plus the finishing steps — and mark each `completed` the moment it's done (exactly one `in_progress` at a time, update after each task, never batch at the end). This gives the user a live, checkbox view of generation progress.
