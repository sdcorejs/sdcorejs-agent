...
# Verify-Before-Done — Acceptance Criteria Gate
...
- Tasks where no spec was written (the workflow's brainstorm/clarify path can bypass spec for tiny scope — in that case this skill skips with a warning)
...
The agent MUST NOT claim "done", invoke `sdcorejs-auto-docs` with a complete status, or trigger `sdcorejs-commit` for a feature commit until:
...
### MUST DO
- Run BEFORE `sdcorejs-auto-docs` at end of any feature-writing skill
...
- Allow `sdcorejs-auto-docs` to write "status: done" while criteria fail
...
## Cross-references
- `_shared/auto-docs` — runs AFTER this skill; this skill blocks auto-docs from claiming "done" prematurely
- `_shared/fix-loop` — invoked when this skill finds failed criteria
- `_shared/auto-task-tracker` — open criteria flow into the living TODO until resolved
