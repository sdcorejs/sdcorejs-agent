...
description: Generate code for a Next.js landing site after `sdcorejs-execute-plan` dispatches an approved Next.js plan, OR as the single entry point for direct build-website code-gen requests whose requirements are already confirmed. Loads the matching on-demand pack under `_refs/nextjs/build-website/write-code/` (per-pack trigger catalog is in the body): init-site, theme, pages-and-blocks, seo, og-preview, i18n, caching, responsive, contact-form, content-quality. Triggers - "bootstrap website", "choose theme", "add a page / add section", "set up SEO / sitemap", "OG image / social preview broken", "add language / i18n", "caching / ISR", "responsive broken / mobile broken", "contact form / form does not send email", "review content / thin content", plus generic "generate code", "go ahead", or localized equivalents. To audit an EXISTING site use `sdcorejs-review` instead. NOT for product-only docs or traceability audits; use `sdcorejs-product`. After completion runs the mandatory tail chain (sdcorejs-test -> sdcorejs-review -> repair-loop -> sdcorejs-comment-code -> sdcorejs-product when user-visible feature traceability is needed -> sdcorejs-ship (verify-before-done mode) -> sdcorejs-ship (branch-ready mode) -> auto-docs tail ref -> write-user-guide -> auto-task-tracker tail ref -> memories). Runtime-localized.
