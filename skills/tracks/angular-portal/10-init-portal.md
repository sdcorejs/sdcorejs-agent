...
## Source of truth — `@sd-angular/core` version

This skill uses the placeholder **`<CORE_VERSION>`** wherever `@sd-angular/core` version appears (package.json template, tree diagrams, checklist, commit message example).

**BEFORE generating any file**, the agent MUST:

1. Read [`_refs/core-version.md`](./_refs/core-version.md) and extract the `currentVersion` field
2. Substitute every occurrence of `<CORE_VERSION>` in this skill's output with that value
3. NEVER hardcode a literal version string in generated `package.json` / commit message / verification text

This makes a version bump a 1-file edit: change `_refs/core-version.md` and every future portal generated by this skill picks it up. Do NOT find-replace `<CORE_VERSION>` inside this skill file itself — the placeholder is intentional.

If `_refs/core-version.md` is missing or malformed, STOP and ask the user — never guess the version.
...
```
portal-new/
├── package.json                   # @sd-angular/core: <CORE_VERSION> (npm)
...
- ✅ `@sd-angular/core@<CORE_VERSION>` (npm registry version, NOT tgz)
...
- [x] package.json uses npm version of @sd-angular/core: `"<CORE_VERSION>"`
- [x] Core version source of truth documented in `skills/tracks/angular-portal/_refs/core-version.md`
...
Pin @sd-angular/core to npm version <CORE_VERSION> (no local tgz dependency).
