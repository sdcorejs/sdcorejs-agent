...
3. Decide whether to regenerate `_refs/angular-portal/sdcorejs-angular/**` from the new source — only needed if API surface changed between versions. If yes, also bump the `Library version: @sd-angular/core@X.Y.Z-beta.N` line at the top of each catalog file.
