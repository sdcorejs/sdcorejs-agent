# Entity Skeleton Templates — init-entity (angular-portal-write-code)

This file holds the canonical Angular code templates the init-entity reference pack ([`init-entity.md`](../write-code/init-entity.md), loaded on demand by the `angular-portal-write-code` orchestrator) generates. The pack picks which template to use based on the chosen layout variant; the actual code lives here so the pack stays concise.
