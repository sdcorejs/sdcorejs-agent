...
- In explicit technical-prototype mode, an approved workflow/custom button is
  mock-first only when `mock-service` is approved; otherwise follow the
  approved service contract. Never infer extra actions from prototype mode.
