...
Code templates referenced from the init-portal reference pack [`init-portal.md`](../write-code/init-portal.md) (loaded on demand by the `angular-write-code` orchestrator). The pack owns the decision logic, rules, and verification flow; this file holds the literal file content to write into the starter.
...
Sidebar branding + menu. The `sidebar.logoUrl` is served from `public/logo.png` (file copied verbatim from `_refs/angular/assets/logo.png` during generation — replace per project without touching code).
