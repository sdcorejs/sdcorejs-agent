...
## When invoked
- Automatic step 3 of "Full build" dispatch (between `theme.md` and `pages-and-blocks.md`)
- User says "add English", "<localized text>", "setup i18n", "language switcher"
...
| `src/app/[locale]/layout.tsx` | Locale-aware root layout (already exists from `init-site.md`) |
...
```json
{
  "nav": {
    "home": "<localized text>",
    "about": "<localized text>",
    "products": "<localized text>",
    "contact": "<localized text>"
  },
  "common": {
    "learnMore": "<localized text>",
    "contactUs": "<localized text>",
    "viewAll": "<localized text>",
...
ALWAYS keep keys symmetric across locales. If a key exists in VI, it MUST exist in EN (and vice versa). Missing-key warnings are a Critical finding in `sdcorejs-review`.

**Parity check is enforced at build time** by `scripts/check-i18n-parity.ts` (installed by `content-quality.md`). It diffs the key sets of `vi.json` ↔ `en.json` and the file sets of `content/vi/` ↔ `content/en/`, then fails `npm run build` if they diverge. Run manually with `npm run check:i18n`. See `content-quality.md` Part 1 for the script and the broader bilingual quality rules (no machine translation, per-locale keyword research, etc.).
  ...
  return (
    <div className="flex items-center gap-2 text-sm">
      <button
        ...
        className={currentLocale === 'vi'<localized text>'font-semibold' : 'text-neutral-500'}
        aria-current={currentLocale === 'vi'<localized text>'true' : undefined}
      >
        VI
      </button>
      ...
      <button
        ...
        className={currentLocale === 'en'<localized text>'font-semibold' : 'text-neutral-500'}
        aria-current={currentLocale === 'en'<localized text>'true' : undefined}
      >
        EN
      </button>
    </div>
...
UI strings live in JSON; long-form content (product list, articles) lives in `src/content/<locale>/` (per `pages-and-blocks.md`). Both need translation. Convention:
...
format.dateTime(new Date(), { dateStyle: 'long' });      // "<localized text>" (VI) / "May 16, 2026" (EN)
format.number(1500000, { style: 'currency', currency: 'VND' });   // "<localized text>" / "<localized text>"
format.relativeTime(-2, 'hour');                          // "<localized text>" / "2 hours ago"
```

Use `next-intl` for locale-bound UI output. For shared non-locale helper logic
such as parse/diff/rounding/paging/filtering/query params, read
`_refs/shared/sdcorejs-utils.md` and prefer `@sdcorejs/utils` instead of adding
bespoke helpers. If both appear in one feature, document the split: `next-intl`
formats what the user sees; `@sdcorejs/utils` handles pure reusable logic.

For pluralization, use ICU MessageFormat syntax in JSON:
```json
"itemCount": "<localized text>"
```
...
## Localized pathnames in `seo.md` sitemap
...
## Cross-references
- Sitemap: `seo.md` includes per-locale alternates derived from routing config
- Localized pathnames: `pages-and-blocks.md` uses `Link` from `@/i18n/routing`
- Theme: `theme.md` already loads `Be Vietnam Pro` with `vietnamese` subset
- Static generation: `setRequestLocale` interacts with `"use cache"` in `caching.md`
- **Bilingual parity + content quality**: `content-quality.md` installs the `check:i18n` script, enforces translation-quality rules (no machine translation), per-locale keyword research, and minimum content lengths
