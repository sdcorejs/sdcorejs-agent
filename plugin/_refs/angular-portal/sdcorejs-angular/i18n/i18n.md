# i18n (`@sd-angular/core/i18n`)

**Type**: Secondary entry point (service + pipe + types + catalogs + tooling)
**Import path**: `@sd-angular/core/i18n`
**Public to portals**: ❌ NOT re-exported from the main `@sd-angular/core` barrel — Core UI internal only. Portals that need the API import it explicitly from `@sd-angular/core/i18n`.
**Library version**: `@sd-angular/core@19.0.0-beta.105`

## One-line purpose
Bilingual+ runtime (default `vi/en/ja/ko/zh`) for every user-facing string inside `@sd-angular/core`, with a signal-based `I18nService`, a `pure: true` `| translate` pipe, type-safe key catalog derived from `EN_MESSAGES`, and an opt-in custom-language hook for portals.
...
```typescript
/**
 * Built-in languages. Defined in `@sd-angular/core/models` (leaf entry point),
 * re-exported from `@sd-angular/core/i18n` for convenience.
 */
...
> **Why is `Language` in a separate entry point?** Both `@sd-angular/core/configurations` and `@sd-angular/core/i18n` need the type. Since `i18n` already imports `SD_CORE_CONFIGURATION` from `configurations`, having `configurations` import a type from `i18n` would create a cross-entry-point cycle (ng-packagr build failure). Moving `Language` to a leaf entry point (`@sd-angular/core/models`) lets both consume it without cycle.
...
### `ISdCoreConfiguration.language` (from `@sd-angular/core/configurations`)
...
- **Type safety for custom catalogs** — cast at the consumer (configuration uses loose `Record<string, string>` to keep its entry point leaf):
  ```typescript
  import type { I18nCatalog } from '@sd-angular/core/i18n';
  // ... configuration:
...
```
i18n parity OK (313 keys × 5 languages)
```

### `npm run check:i18n`
Scans `projects/sd-angular/**/*.{ts,html}` for raw Vietnamese diacritics (the regex covers all combinations of `À-ỹ` + `Đ`/`đ`). Fails CI on any hit outside the whitelist.
...
import { I18nService, TranslatePipe } from '@sd-angular/core/i18n';
...
```typescript
import type { I18nCatalog, ISdCoreConfiguration } from '@sd-angular/core/i18n';
...
1. Add to `Language` union in `projects/sd-angular/models/src/language.model.ts`:
   ```ts
   export type Language = 'vi' | 'en' | 'ja' | 'ko' | 'zh' | 'fr';
   export const SUPPORTED_LANGUAGES: readonly Language[] = ['vi', 'en', 'ja', 'ko', 'zh', 'fr'] as const;
   ```
2. Create `projects/sd-angular/i18n/src/fr.ts` with `FR_MESSAGES: Record<keyof typeof EN_MESSAGES, string>` — TypeScript enforces parity.
3. Register in `projects/sd-angular/i18n/src/i18n.messages.ts`:
   ```ts
...
## Related
- `@sd-angular/core/models` — leaf entry point owning the `Language` type + `SUPPORTED_LANGUAGES` constant. Imported by both `configurations` and `i18n`.
- `@sd-angular/core/configurations` (`SD_CORE_CONFIGURATION`) — DI token where `language` is set.
- `@sd-angular/core/utilities/models` (`MaybeAsync`) — used elsewhere but **not** by i18n (custom language is sync-only by design).
- `@sd-angular/core/utilities/extensions` (`BrowserUtilities.upload`) — example of the DI-less hardcode pattern (see Example #7).
- `scripts/check-i18n.mjs` + `scripts/check-i18n-parity.mjs` — CI guards.
- `projects/sd-angular/i18n/src/{vi,en,ja,ko,zh}.ts` — catalog source files.
