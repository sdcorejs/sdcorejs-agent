...
2. **Write the project brief:** run `sdcorejs-explore (summary-refresh)` in write-approved context to create `.sdcorejs/summary.md` (domain, stack, generated kernel layout, schema-per-module convention, reuse cheatsheet, current git HEAD, freshness metadata, stack profile evidence, and redaction status) - same pattern as the angular `init-portal` tail. The next session and `init-module` preflight read this instead of re-scanning blind.
