...
| **Pinned** (e.g. `@sd-angular/core`) | locked by reference doc | DO NOT upgrade without re-pinning ref; see `_refs/core-version.md` |
| **Critical security** | npm audit High/Critical | Out-of-band fix, highest priority |

For `@sd-angular/core` specifically — read `skills/tracks/angular-portal/_refs/core-version.md`. If the target project pins to a version, do NOT upgrade unilaterally; ask the user and update the reference doc together.
...
- `@sd-angular/core` is pinned via `_refs/core-version.md` — coordinate
...
- Update `_refs/core-version.md` if `@sd-angular/core` is bumped
