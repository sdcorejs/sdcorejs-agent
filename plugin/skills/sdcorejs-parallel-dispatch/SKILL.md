...
description: Parallel execution gate and subagent fan-out discipline. Use after `sdcorejs-execute-plan` asks sequential vs parallel and the user chooses parallel, or when the user explicitly says "run in parallel", "split into subagents", "fan out", or similar. Decides whether the approved work is SEQUENTIAL, PARALLEL-CANDIDATE (independent same-kind units), or ROLE-SPLIT (one feature across product/design/backend/frontend/test-QC), then executes the safe parallel path itself: decompose, brief, dispatch subagents, review returned work, merge/synthesize, and run global verification. Applies to angular, nestjs, nextjs, product, design, test, and generic harness work. Runtime-localized.
