...
## When invoked
- **MANDATORY** automatic invocation BEFORE `sdcorejs-branch-ready` → `sdcorejs-auto-docs` at the end of every code-writing skill (`write-code` — the `sdcorejs-angular` orchestrator and the reference packs it loads, including `actions` — and `sdcorejs-test`)
