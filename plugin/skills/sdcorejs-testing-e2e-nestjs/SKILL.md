...
description: Use to write end-to-end tests for a NestJS backend — `@nestjs/testing` + `supertest` against a real HTTP server with a real (test) PostgreSQL via `testcontainers` or `pg-mem`. Covers auth flow, ZodValidationGuard, AuthGuard + HasPermission, transactions, and bilingual error messages. Inherits cross-track principles from `testing/philosophy.md`. Triggers - "e2e nestjs", "test backend flow", "supertest", "test API endpoint with real DB", or automatic invocation after a NestJS feature lands. Bilingual (VI/EN).
...
## When invoked
- After a `<entity>.controller.ts` + `<entity>.service.ts` are wired
- User says "test API endpoint", "supertest", "e2e nestjs"
- Before a new endpoint lands in main
...
- Integration: `testing/integration/nestjs.md` (lighter weight)
