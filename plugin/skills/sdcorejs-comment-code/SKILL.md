...
description: Apply code comments at the level chosen in the consolidated FINISH GATE (`_refs/shared/finish-gate.md`) or ask the level when invoked directly. Loads `_refs/orchestration/tail/comment-code.md` for the `skip` / `simple` / `medium` / `full` rulesets. Use in the code-generation tail after `sdcorejs-review` / `sdcorejs-repair-loop` and before `sdcorejs-ship (verify-before-done mode)`, or when the user says "add comments", "comment the code", "comment level", or localized equivalents. Applies to angular, nestjs, nextjs. Runtime-localized.
