...
description: Use to build a REAL working contact form — client-side validation with zod + react-hook-form (or plain state), API route POST `/api/contact`, Resend (or SendGrid) email integration, in-memory rate limiting, bilingual error messages, accessible success/error UI. Replaces the fake `setTimeout` stub pattern common in starter projects. Triggers - "contact form", "biểu mẫu liên hệ", "form không gửi email", "fix fake form", or automatic step of `07-write-code` for any site that includes a contact section. Bilingual (VI/EN).
