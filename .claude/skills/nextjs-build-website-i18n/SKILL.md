...
Why localized pathnames: VI users see `myhoa.vn/san-pham`, EN users see `myhoa.vn/about-us`. Better UX + better SEO (each locale's URLs match its content). The Next.js + next-intl machinery handles the URL ↔ canonical route mapping.
