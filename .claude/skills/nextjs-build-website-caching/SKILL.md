---
name: nextjs-build-website-caching
description: Use to set up or tune the Next.js caching strategy for a landing site — applies the **30-minute ISR default** to all pages, configures on-demand revalidation for content updates and contact form submissions, and provides a decision tree for SSG vs ISR vs SSR. Triggers - "set up caching", "cấu hình cache", "cache lâu/ngắn quá", "ISR settings", "revalidate", or automatic step of `07-write-code` after pages exist. Bilingual (VI/EN).
...
## When invoked
- Automatic step of `07-write-code` after pages are generated
...
- **Standard everywhere**: Matches what `website_myhoa` ships and what Vercel's official docs recommend for landing pages.
...
If user picked Cloudflare in `02-clarify-requirements` → confirm 30-min ISR is acceptable (it is for most landing sites) OR fall back to longer windows + on-demand only.
...
- Outputs to: `_shared/verify-before-done` (caching-rendering-mode is verifiable acceptance criterion)
