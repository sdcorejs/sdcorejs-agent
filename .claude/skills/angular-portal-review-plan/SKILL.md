...
- [ ] Final step references `_shared/auto-docs` (mandatory) and optionally `_shared/memories`
...
### 4. Handle response
- **Approve** ("OK", "duyệt", "approve", "go ahead", "tiến hành", "generate"): hand off to `07-write-code` with the approved plan as input
- **Request changes** ("đổi step N", "sửa", "amend", "change"): edit the plan, re-run self-review, re-present (loop max 3 rounds; if still not approved, suggest going back to `05-plan` or `04-review-spec`)
- **Abort** ("hủy", "dừng", "cancel", "stop"): stop the workflow. Optionally invoke `_shared/memories` if the user abort surfaced a durable lesson (e.g. "user prefers smaller batches; plan was too big")
...
- `07-write-code` — runs immediately after approval; executes the plan
- `04-review-spec` — sibling gate, one stage earlier in the workflow
- `_shared/memories` — capture durable lessons learned during review
