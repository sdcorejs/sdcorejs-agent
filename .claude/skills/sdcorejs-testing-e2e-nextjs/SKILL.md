---
name: sdcorejs-testing-e2e-nextjs
description: Use to write E2E tests for a Next.js landing site using Playwright — covers SSR-aware assertions, locale-prefixed routing (`/vi/san-pham` vs `/en/products`), form submissions, OG image rendering, sitemap/robots presence, and dynamic OG image rendering. Inherits cross-track testing principles from `testing/philosophy.md`. Triggers - "e2e test cho nextjs", "Playwright nextjs", "test landing page", "test luồng contact", "viết test e2e nextjs", or automatic invocation after `07-write-code` completes a Next.js feature. Bilingual (VI/EN).
...
Read `testing/philosophy.md` first for principles (test pyramid, what to mock, AAA). This skill is the HOW for Next.js.

## When invoked
- After `07-write-code` completes a Next.js feature (automatic)
- User says "viết e2e cho /lien-he", "test luồng contact", "Playwright nextjs"
- Adding a new page or section that has user interactions
...
### MUST DO
- Inherit principles from `testing/philosophy.md` (AAA, test names, what to mock)
