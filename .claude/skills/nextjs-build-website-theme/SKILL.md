...
description: Use to apply the design system (palette + typography + spacing + design tokens) to a freshly-initialized Next.js site based on the industry vibe picked in `01-brainstorm`. Triggers - "pick theme", "chọn theme", "set up colors / fonts", "design system", or automatic step of `07-write-code` after `10-init-site`. Bilingual (VI/EN).
...
## When invoked
- Automatic after `10-init-site` in a "Full build" dispatch
...
ALWAYS use `next/font/google` (already wired in `[locale]/layout.tsx` from `10-init-site`). If pairing requires 2 families, add a second font in the layout.
