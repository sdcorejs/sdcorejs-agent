...
- **Auto-invoked** at the comment phase of every code-writing workflow:
  - After `50-review-code` and `_shared/fix-loop` (if findings existed)
  - BEFORE `51-write-comments` (Angular) / NestJS/NextJS equivalents
  - BEFORE `_shared/verify-before-done`
...
- Angular: `skills/angular-portal/51-write-comments.md`
...
The orchestration: `_shared/comment-code` confirms level=full → dispatch to `51-write-comments` → return.
...
> "Bỏ qua comment theo lựa chọn của bạn. Hand off `_shared/verify-before-done`."

## Cross-references
- `51-write-comments.md` (angular-portal) — FULL level implementation; this skill dispatches to it
- `_shared/auto-docs` — runs after this skill; the chosen level + count goes into the session summary
- `_shared/verify-before-done` — runs after this skill; comment count is NOT an acceptance criterion (commenting is a styling concern, not feature correctness)
- `_shared/fix-loop` — if review surfaces missing-comment findings AND the user picked `skip`, those findings are auto-deferred (the user's choice overrides)
