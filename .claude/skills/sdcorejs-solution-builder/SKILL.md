...
3. **Seed the approved product contract** *(`sdcorejs-product`, action `seed-from-approved-spec`)* - Immediately after the approved spec snapshot, project the approved requirement IDs into the target's existing product-doc convention and create the collision-safe active ledger/history identity. Preserve the spec path/hash and emit `product_context`; do not claim implementation or test coverage yet.

4. **Plan the build + check again** *(`sdcorejs-plan` = **GATE 2**)* - Lay out the build steps against the exact seeded contract/revision and confirm again, in plain words ("Here's how I'll build it - good to go?"). Same rule: explicit approval only, silence is not consent. On approval, `sdcorejs-plan` writes the approved plan snapshot itself.

5. **Prepare screen design** *(`sdcorejs-design`)* - Create `design/` handoff artifacts from the approved user stories: flows, design specs, editable wireframes, and PNG exports when a renderer is available or the user asks for previews. To the user: "preparing screen references before building."

6. **Build only the selected complexity** *(`sdcorejs-execute-plan` -> `sdcorejs-parallel-dispatch` when approved)* - Invoke the owning skill with the selected `complexity_level`, `profile`, `topology`, `layout`, and approved spec/plan/design handoff. `sdcorejs-execute-plan` must still ask the user whether to run sequentially or in parallel. Branch the build by complexity:
   ...
   - `full secure stack` / `enterprise/full-stack`: use `profile=simple` unless enterprise was explicitly selected, `topology=single solution-root`, `layout=product/design/backend/frontend/test/.sdcorejs`, admin-first enabled, and recommend parallel role-split unless the user prefers sequential. To the user this is just "building the part that stores your data and the screens you'll use, together, including the in-app account and role management screens." `sdcorejs-parallel-dispatch` handles the frozen contract and design/backend/frontend/test role split. Product traceability is not a build-wave role; it runs after deterministic implementation/test fan-in.

   If subagents are unavailable, or Mode B escalates, use the sequential fallback: freeze the same contract, run `sdcorejs-nestjs` under `backend/`, `sdcorejs-angular` under `frontend/`, `sdcorejs-test` under `test/`, then `sdcorejs-review` and `sdcorejs-repair-loop` when needed. Do not call product sync or ship yet; first finish every write-producing step below.
...
8. **Add login** *(`sdcorejs-auth`, conditional)* - Run only for `full secure stack` / `enterprise/full-stack`, or when login/auth was explicitly approved for a lower complexity level. Wire authentication into the frontend and backend only when it belongs to the approved complexity.

9. **Write the start guide** *(`sdcorejs-run-guide`, conditional)* - Emit `START.md` only for runnable deliveries. Match the guide to the selected complexity and actual stack.

10. **Finish all write-producing tails** - Run `_refs/orchestration/tail/auto-docs.md`, approved user/technical documentation, `_refs/orchestration/tail/auto-task-tracker.md`, and `sdcorejs-explore (memories mode)` when durable knowledge surfaced. These writes must complete before final product synchronization.

11. **Synchronize and audit the product contract** *(`sdcorejs-product`)* - After implementation/test fan-in and every other write, run `traceability-sync` against actual integrated paths and current evidence. Re-run affected global verification on the post-sync state, then run `audit-readonly` with a zero-write before/after proof. Any relevant later write stales this context and requires sync/audit again.

12. **Verify the selected delivery** *(`sdcorejs-ship (verify-before-done mode)` + conditional smoke)* - Consume the final `audit-readonly` product context when a product contract exists, but independently re-verify every approved criterion. Run Docker stack checks only when Docker packaging was built. Minimum full-stack checks:
   ...
   If Docker is unavailable, a port is occupied, or the smoke cannot be automated, say exactly what was not verified and do **not** claim the packaged stack is runnable. For lower complexity levels, report the matching local/manual smoke that actually ran and avoid packaged-stack language. Finish with `sdcorejs-ship (branch-ready mode)` as the final read-only gate. No writes after branch-ready; if a write occurs, rerun product sync/audit, ship verification, and branch-ready.

13. **Tell the user how to run it** - Close in plain language only after the relevant checks above have actually run. Point them at `START.md` when it exists and describe only the stack and checks actually produced.
...
- If interruption happens after packaging but before stack verification, treat the package as unverified until the stack smoke in Step 12 runs in the resumed session.
