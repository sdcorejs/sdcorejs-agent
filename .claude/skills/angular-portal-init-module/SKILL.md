...
A generic dropdown component that wraps `sd-select` and `BaseService.register(url).search/all`. Generated ONCE per module, then reused by every `<entity>-select` (e.g. `customer-select`, `project-select`) so dropdowns inside CREATE/UPDATE forms stay consistent and the `BaseService` boilerplate isn't repeated.
