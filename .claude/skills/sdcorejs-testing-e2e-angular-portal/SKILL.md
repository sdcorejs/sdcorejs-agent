...
description: Use after Angular-portal code is generated to write E2E tests covering the happy path of the just-built feature (list rendering, create/update/detail navigation, form validation, workflow actions). Detects whether the target project uses Cypress or Playwright and follows that framework's conventions. Inherits cross-track principles from `testing/philosophy.md`. Triggers - "viết e2e test angular", "thêm e2e cho", "test luồng tạo mới angular portal", "test happy path angular", "Cypress angular portal", "Playwright angular portal", or automatic invocation after `angular-portal-write-code` completes. Bilingual (VI/EN).
