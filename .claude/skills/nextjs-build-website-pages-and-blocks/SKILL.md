...
description: Use to compose pages from reusable section blocks (hero, features, testimonials, pricing, FAQ, gallery, CTA, contact form) and to enforce the "no hardcoded strings in components — all content in content/<locale>/" rule. Triggers - "add a page", "thêm trang", "tạo section", "compose home page", or automatic step of `07-write-code` after `11-theme` + `15-i18n` are wired. Bilingual (VI/EN).
...
## When invoked
- Automatic after `11-theme` + `15-i18n` in a "Full build" dispatch
...
Prerequisites:
- Theme tokens exist (`11-theme` done)
- next-intl is wired (`15-i18n` done, or at least messages files exist)
...
This is the #1 deviation from `website_myhoa` baseline. Without this rule, i18n + CMS integration require a full refactor.
...
| ContactForm | `contact-form.tsx` | Inline form (uses `18-contact-form`) | `(none — form owns its strings via i18n)` |
...
// Per-page metadata (handled by 13-seo)
...
5. Image rendering uses `next/image` with explicit `sizes` (see `17-responsive`).
...
4. Wire `generateMetadata` (see `13-seo`).
5. Add the page to `src/app/sitemap.ts` automatically (sitemap reads from a route registry).
6. Add per-locale path to `src/i18n/routing.ts` `pathnames` (see `15-i18n`) if locales should use different URLs (e.g. `/san-pham` (VI) vs `/products` (EN)).
...
- Use tokens from `11-theme` for color / spacing — never raw hex/Tailwind colors
...
- Apply `"use cache"` + `cacheLife()` per page (from `16-caching`)
- Wire `generateMetadata` per page (from `13-seo`)
...
## Anti-patterns
- **Hardcoded products array in ProductsSection.tsx** — the website_myhoa baseline. Solution: move to `content/<locale>/products.ts`.
...
## Cross-references
- Theme tokens: `11-theme`
- Image strategy + responsive: `17-responsive`
- i18n & content JSON: `15-i18n`
- SEO metadata: `13-seo`
- Caching: `16-caching`
- Contact form section impl: `18-contact-form`
