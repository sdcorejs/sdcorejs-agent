...
| **Pinned** (e.g. `@sdcorejs/angular`) | locked by reference doc | DO NOT upgrade without re-pinning ref; see `_refs/angular/core-version.md` |
| **Critical security** | npm audit High/Critical | Out-of-band fix, highest priority |

For `@sdcorejs/angular` specifically — read `_refs/angular/core-version.md`. If the target project pins to a version, do NOT upgrade unilaterally; ask the user and update the reference doc together.
...
- `@sdcorejs/angular` is pinned via `_refs/angular/core-version.md` — coordinate
...
- Update `_refs/angular/core-version.md` if `@sdcorejs/angular` is bumped
