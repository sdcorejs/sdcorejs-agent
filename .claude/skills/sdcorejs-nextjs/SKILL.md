...
description: Next.js website code executor for approved site/module work with confirmed requirements and an explicit feature set. Use for approved bootstrap, theme, pages/sections, SEO/OG, i18n, caching/ISR, responsive, contact, analytics, CMS, or content-quality scope. Basic sites do not imply i18n or contact. Loads only approved reference packs; use sdcorejs-review for existing-site audits. Runs mandatory finish tail. Runtime-localized.
...
Before dispatch, call `_refs/nextjs/execution-contract.mjs` with the central
registry identity, semantic site/module/portal-composition owner, explicit
website profile, approved requirement features, and requested features. Stop on
an unresolved owner or unapproved feature. A module/site artifact never falls
back to the portal repository; portal ownership exists only for explicit
portal-composition scope.

Before writing any helper, formatter, validator, mapper, paging/filter helper, random-id helper, query-param helper, upload/download helper, clipboard/browser helper, API-route utility, hook utility, or `src/lib/utils.ts` addition, read `_refs/shared/sdcorejs-utils.md` and reuse `@sdcorejs/utils` when it covers the behavior. Keep `next-intl` only when `i18n` is approved; use `@sdcorejs/utils` for shared pure helper behavior. The package must be a direct target-project dependency before generated code imports it.
...
authorize a feature; the existence of a "full build" sequence does not.

| User intent / plan item | Reference pack to read | Phase |
|---|---|---|
| Approved new-site baseline | the approved subset of the sequence below | Profile build |
...
## Approved-feature execution sequence

For a new site, apply only approved features in dependency order:

```
1. init-site.md          ← only when `init-site` is approved
2. theme.md              ← only when `theme` is approved
3. i18n.md               ← optional `i18n`; before localized pages
4. pages-and-blocks.md   ← only approved pages/blocks
5. seo.md                ← optional `seo-basic` / `seo-advanced`
6. og-preview.md         ← optional `og-preview`
7. caching.md            ← optional `caching`
8. responsive.md         ← approved responsive verification
9. contact-form.md       ← optional `contact-form`
10. content-quality.md   ← optional `content-quality`
11. analytics/CMS pack  ← only an explicitly approved integration contract
```

The `basic` website profile does not include `i18n`, `contact-form`,
`seo-advanced`, `analytics`, or `cms-integration`. Never add them merely because
the site is new. The `standard` and `full` profiles authorize their declared
sets only after explicit profile approval; an approved requirement may add an
individual feature without changing the whole profile.
...
Execute only real dependency edges. An approved i18n feature precedes localized
pages; an unapproved i18n feature creates no step or dependency. SEO/OG/caching
may fan out after their approved page inputs exist.
...
- Resolve `approved_features`, artifact identity, and semantic owner with
  ...
  `_refs/shared/system-registry.json` and block portal fallback for site/module
  artifacts
...
- Dispatch only approved packs in dependency order; i18n/contact/advanced
  SEO/analytics/CMS are absent when not approved
...
- Generate i18n, contact, advanced SEO, analytics, CMS, or another integration
  merely because a new/basic site is being built
- When `contact-form` is approved, ship a fake `setTimeout` form instead of its
  real validated provider boundary
...
## Anti-patterns
- Generating pages before their approved theme/i18n dependencies
- Treating optional technical SEO, OG, contact, analytics, or CMS as approved
  simply because a pack exists
- Treating an unapproved contact form as a baseline website requirement
