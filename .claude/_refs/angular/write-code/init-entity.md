...
- Follow this import order in every component/service file:
  ...
  2. RxJS (if used)
  3. Third-party / library (Core UI imports — see `_refs/angular/core-version.md` for current package name; templates use `<CORE_UI_PACKAGE_NAME>/...` placeholder)
...
5. **Import Organization**
   ...
   - [ ] Ensure no legacy imports like `from 'sd-angular'` exist; use `<CORE_UI_PACKAGE_NAME>/...` paths (substituted from `_refs/angular/core-version.md`)
...
- **Worked example (Product entity)** — `_refs/angular/templates/example-product.md`
