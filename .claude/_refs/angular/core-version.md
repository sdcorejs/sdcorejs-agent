...
> The catalog under `_refs/angular/sdcorejs-angular/**` documents the surface using the **new** package name (`@sdcorejs/angular/...`). For a legacy project, swap the prefix back to `@sd-angular/core/...` — entry-point sub-paths are identical.
...
3. Decide whether to regenerate `_refs/angular/sdcorejs-angular/**` from the new source — only needed if API surface changed between versions. If yes, also bump the `**Library version**: @sdcorejs/angular@X.Y.Z` line at the top of each catalog file.
