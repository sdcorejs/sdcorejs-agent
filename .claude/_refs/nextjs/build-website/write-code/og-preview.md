...
The static fallback is already referenced from `src/lib/seo.ts` via the `buildMetadata` factory (`seo.md`). Verify:
```typescript
// from seo.md's buildMetadata
const ogImage = image ?? absoluteUrl('/og-default.png');
```

### Step 2 — Dynamic OG (Standard / Full tier)

Next.js App Router auto-renders any `opengraph-image.tsx` file colocated with a route. It uses `@vercel/og` (already installed in `init-site.md`) to generate a PNG on demand and cache it.
...
`seo.md`'s `buildMetadata` already sets `twitter.card: 'summary_large_image'` and `twitter.images: [ogImage]`.
...
Dynamic OG images are generated on first request per route then cached. With 30-min ISR (from `caching.md`), the image is regenerated alongside the page. To verify caching is working:
...
## Cross-references
- Metadata factory: `seo.md` (provides `buildMetadata` with OG defaults)
- Theme tokens: `theme.md` (OG image uses `theme.brand.primary` for background)
- Company config: `src/config/company.ts` (name, tagline, domain)
- Caching: `caching.md` (OG images cached per route alongside the page)
