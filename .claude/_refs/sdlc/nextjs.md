...
Last numbered step references the tail-call chain (sdcorejs-test -> optional sdcorejs-simplify -> affected focused tests -> sdcorejs-review -> sdcorejs-repair-loop -> sdcorejs-documentation (code-documentation mode, automatic) -> sdcorejs-documentation (write-technical-doc mode, if approved) -> _refs/orchestration/tail/auto-docs.md -> sdcorejs-documentation (write-user-guide mode, if approved) -> _refs/orchestration/tail/auto-task-tracker.md -> sdcorejs-explore (memories mode) -> sdcorejs-ship (verify-before-done mode) -> sdcorejs-ship (branch-ready mode as the final read-only gate)). Simplification is visible opt-in, never automatic, and any write invalidates affected earlier evidence. The reviewer checks the chain is implicit, not omitted, and that no writes happen after branch-ready unless branch-ready is run again.
