...
Reference output from a complete init-entity run (the `angular-portal-write-code` orchestrator with the [`init-entity.md`](../write-code/init-entity.md) pack) against `product`. Use this to verify the shape and conventions of the generated bundle; do NOT copy-paste — let the orchestrator emit the parameterized templates from `entity-skeleton.md`.
