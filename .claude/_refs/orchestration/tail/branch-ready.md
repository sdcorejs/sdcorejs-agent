...
Reference body for `sdcorejs-ship (branch-ready mode)`. Load this file only after the skill triggers.

## Purpose
Acceptance criteria passing (the `sdcorejs-ship (verify-before-done mode)`) doesn't mean the branch is ready to ship. Debug logs left in, a `.only` on a test, commented-out experiments, a stray `.env` — these all pass acceptance criteria but leak into PRs and embarrass on review. This reference is the last sanity sweep before `sdcorejs-git` commit mode.

It does NOT modify code. It surfaces issues so the user decides — fix in place, ignore, or defer.

## When invoked
- **Automatic**: in the SDLC tail-call chain, right after `sdcorejs-ship (verify-before-done mode)` returns ✅ and before `sdcorejs-git (commit mode)`; also invoked as Step 3 by `sdcorejs-ship`
...
|---|---|
| `$CHANGED_COUNT > 8` AND `$MODULE_COUNT > 2` (both conditions simultaneously) | ⚪ Many modules affected — consider `sdcorejs-review` with the `architecture` dimension before merging |
...
- [architecture] 12 files across 4 modules → consider `sdcorejs-review` (architecture dimension)
...
**🟢 READY** — all blockers clear; lint/build/test pass.
  → Hand off to `sdcorejs-git (commit mode)` next.
...
**🟡 READY WITH WARNINGS** — blockers clear but N warnings; user acknowledged via "I know, ship anyway" or similar.
  → Hand off to `sdcorejs-git (commit mode)`.
```
...
This skill runs at the PRE-COMMIT boundary. For PR-time checks (after CI), use `sdcorejs-git (PR mode)` which has its own gate.
...
- Suggest the next skill (`sdcorejs-git (commit mode)` / `sdcorejs-git (PR mode)`) when verdict is READY
...
- Running this BEFORE `sdcorejs-ship (verify-before-done mode)` — wrong order; acceptance comes first
...
## Cross-references
- `sdcorejs-ship (verify-before-done mode)` — runs before; acceptance-criteria gate
- `sdcorejs-git (commit mode)` — runs after when READY; uses Conventional Commits
- `sdcorejs-ship` — calls this skill as Step 3 in the ship chain; returns here before proceeding to changelog/commit
- `sdcorejs-git (PR mode)` — runs after `sdcorejs-git (commit mode)` when shipping
- `sdcorejs-debug` — if a blocker is a failing test you don't understand
